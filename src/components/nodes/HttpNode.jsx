...
const HttpNode = ({ id, data = {} }) => {
  ...
  const [queryParams, setQueryParams] = useState(() =>
    normalizeList(data.queryParams)
  );
  ...
  const updateList = (list, setList, type) => (idx, field, val) => {
    ...
    setList(next);
    updateNodeData(id, {
      [type]: Object.fromEntries(next.map((i) => [i.key, i.value])),
    });
  };
  ...
  return (
    <div className="http-node border rounded-md shadow-md bg-white">
      <div className="node-header http flex items-center justify-between p-2 bg-blue-100">
        ...
        <div style={{ color: "black" }} className="flex items-center gap-">
          <select
            value={data.method}
      ...
      {!isCollapsed && (
        <div className="node-content p-2">
            ...
            headers.map((h, i) => (
              <div key={i} className="flex gap-2 mb-1">
                <input
                  ...
                  value={h.key}
                  onChange={(e) =>
                    updateList(headers, setHeaders, "headers")(
                      i,
                      "key",
                      e.target.value
                    )
                ...
                <button
                  onClick={() =>
                    removeFromList(headers, setHeaders, "headers")(i)
                  }
                >
                  ✕
                </button>
          ...
          {activeTab === "headers" && (
            <button
              onClick={addToList(setHeaders, "headers")}
            ...
            queryParams.map((p, i) => (
              <div key={i} className="flex gap-2 mb-1">
                <input
                  ...
                  value={p.key}
                  onChange={(e) =>
                    updateList(queryParams, setQueryParams, "queryParams")(
                      i,
                      "key",
                      e.target.value
                    )
          ...
          {activeTab === "queryParams" && (
            <button
              onClick={addToList(setQueryParams, "queryParams")}
              className="text-sm text-blue-600"
            >
              + Add Param
            </button>
            ...
            bodyFields.map((b, i) => (
              <div key={i} className="flex gap-2 mb-1">
                <input
                  ...
                  value={b.key}
                  onChange={(e) =>
                    updateList(bodyFields, setBodyFields, "body")(
                      i,
                      "key",
                      e.target.value
                    )
                ...
                <button
                  onClick={() =>
                    removeFromList(bodyFields, setBodyFields, "body")(i)
                  }
                >
                  ✕
                </button>
          ...
          {activeTab === "body" && (
            <button
              onClick={addToList(setBodyFields, "body")}
              className="text-sm text-blue-600"
            >
              + Add Body Field
            </button>
